# Pro / Basic（专业版 / 常规版）视图切换（v2.2,对齐 4 要素 + showcase）

> SKILL.md Step 4a 调用。模板自带 view-tabs UI + CSS + JS + localStorage 持久化。**生成内容时按下面规则给元素加 class**,常规版(默认)与专业版各看一份合适的内容。
> **标准化定位**:本文件是「常规/专业自动区分」的**唯一规则源**——决策表 + 4 要素标记策略 + Checklist 让标记可重复、不靠每次临场判断。属管线 ③ 发布段,见 [`../../shared/references/design-md-pipeline-sop.md`](../../shared/references/design-md-pipeline-sop.md)。
...
## 4 要素标记策略(v2.2,对齐现行 4 要素结构)

> ⚠️ v2.0 把 7 章节压成 **4 要素**(组件定义 / 行为准则 / 设计属性 / 应用场景),原「7 章节标记策略」作废。下面按现行 4 要素标记。每个 §N 都要常规版自洽 + 专业版无遗漏。
...
### §3 设计属性(最 token-heavy,Pro/Basic 差异最大)
